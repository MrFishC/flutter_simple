...
class _MaterialControlsState extends State<MaterialControls>
  ...
  Widget build(BuildContext context) {
    ...
    //MouseRegion:监听区域内鼠标的进入和退出以及移动轨迹
    return MouseRegion(
      onHover: (_) {//鼠标在区域内移动时的回调
      ...
      child: GestureDetector(
        ...
        child: AbsorbPointer(
          absorbing: notifier.hideStuff,//absorbing:是否吸收指针事件
          child: Stack(
            children: [
  ...
  Widget _buildHitArea() {
    ...
    print("参数信息 ${_latestValue.isBuffering}");
  ...
  _overlayUI() {
    return widget.overlayUI != null
        //AnimatedOpacity :动画组件 可以实现渐入渐出的效果
        ? AnimatedOpacity(
            opacity: notifier.hideStuff ? 0.0 : 1.0,  //透明度 0 - 1
            duration: Duration(milliseconds: 300),
            child: widget.overlayUI //要控制透明度的子组件
    )
        : Container();
  }
}
